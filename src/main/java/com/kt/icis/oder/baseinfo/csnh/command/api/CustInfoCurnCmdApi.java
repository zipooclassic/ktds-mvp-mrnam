...
import com.kt.icis.oder.baseinfo.csnh.command.controller.CustInfoCurnCmdCntr;
 ...
 * @FileName: CustInfoCurnCmdApi
 * @Class 설명 : 고객정보 현행화 처리 데몬 API 작업
 * @작성일 : 2024. 08. 17.
...
@Tag(name = "고객정보현행화처리데몬", description = "고객정보 현행화 처리 데몬-CSNH626D")
public class CustInfoCurnCmdApi {

	private final CustInfoCurnCmdCntr  contrller;
	/**
	 * @Method Name : custInfoCurn
	 * @Method 설명 : 고객정보 현행화 처리 데몬
	 * @작성일 : 2024. 08. 17.
	 * @작성자 : 리그시스템 / 91347489 / 강동순
	 * @param : BiDaemonTmpUseInCmdPyld
	 * @return : BiDaemonTmpUseOutCmdPyld
	 */
	@Operation(summary = "고객정보현행화처리데몬", description = "고객정보 현행화 처리 데몬-CSNH626D")
    @PostMapping(path = "/cust/infocurn")
    public BiDaemonTmpUseOutCmdPyld custInfoCurn(@RequestBody BiDaemonTmpUseInCmdPyld inPyld) {
		BiDaemonTmpUseOutCmdDto rtnDto = new BiDaemonTmpUseOutCmdDto();
		contrller.custInfoCurn();
